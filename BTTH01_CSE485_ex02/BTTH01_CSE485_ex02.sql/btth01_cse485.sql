...
--
-- Máy chủ: 127.0.0.1
-- Thời gian đã tạo: Th9 22, 2023 lúc 03:33 AM
...
ALTER TABLE `baiviet`
  ...
  ADD KEY `khoa_tl` (`ma_tloai`),
  ADD KEY `khoa_tg` (`ma_tgia`);
...
--
ALTER TABLE `baiviet`
  ADD CONSTRAINT `khoa_tg` FOREIGN KEY (`ma_tgia`) REFERENCES `tacgia` (`ma_tgia`),
  ADD CONSTRAINT `khoa_tl` FOREIGN KEY (`ma_tloai`) REFERENCES `theloai` (`ma_tloai`);
COMMIT;
...
/*a */
SELECT tieude, ten_bhat FROM `baiviet` WHERE baiviet.ma_tloai = 2;
/*b. Liệt kê các bài viết của tác giả “Nhacvietplus” (2 đ) */
select * from baiviet WHERE baiviet.ma_tgia =1;

/*c. Liệt kê các thể loại nhạc chưa có bài viết cảm nhận nào. (2 đ)*/ /*INSERT INTO theloai VALUES (10, 'KPOP');*/
SELECT * FROM theloai, baiviet WHERE theloai.ma_tloai = baiviet.ma_tloai IS null;
